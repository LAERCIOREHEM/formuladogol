(function () {
  ...
  function shield(obj, cls = "stats-shield") {
    ...
    if (!src) {
      if (String(cls || "").includes("probability-club-shield")) return "";
      return `<span class="stats-shield-fallback ${escapeAttr(cls)}">${escapeHtml(initials(name))}</span>`;
    }
  ...
  function bindEvents() {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (probabilitySort) {
        state.probabilitySort = probabilitySort.dataset.probabilitySort || "campeao";
        renderProbabilityControls();
        renderProbabilityRanking();
        return;
      }
