...
(function (global, document) {
  ...
  function limparNomeParticipante(nome) {
    return String(nome || "").replace(/\s+/g, " ").trim();
  }

  function loginDoNomeParticipante(nome) {
    return limparNomeParticipante(nome);
  }

  function mensagemWhatsappAcesso(nome, login, pin) {
    const nomeExibicao = limparNomeParticipante(nome) || login || "participante";
    return [
      `Fala, ${nomeExibicao}! ⚽`,
      "",
      "Seu acesso ao Bolão Brasileirão Almoço está liberado:",
      "",
      `Usuário: ${login}`,
      `PIN: ${pin}`,
      "",
      "Acesse:",
      "https://brasileirao2026almoco.com.br/apostas.html",
      "",
      "Entre com esse usuário e PIN para fazer seus palpites da rodada."
    ].join("\n");
  }
  ...
  async function copiarTexto(texto, okMsg) {
    try {
      ...
      status(okMsg || "Mensagem copiada.", "ok");
      return true;
    } catch (_) {
      status("Não consegui copiar automaticamente; selecione e copie a mensagem manualmente.", "warn");
      return false;
    }
  }
  ...
  async function renderAdmin() {
    ...
    const globalAdmin = isAdminGlobal();
    const painelParticipantes = globalAdmin ? `<article class="panel admin-access-card"><div class="panel-inner">
        <div class="kicker">Participantes</div><h2>Criar/renovar acesso</h2>
        <form id="admin-participante" class="admin-form admin-access-form">
          <input type="hidden" id="admin-participante-id">
          <label>Nome do participante <input id="admin-nome" required placeholder="Ex.: CARLINHOS" autocomplete="off"></label>
          <div class="actions"><button class="btn" type="submit">🔑 gerar acesso e mensagem</button></div>
          <div id="admin-acesso-gerado" class="access-message-box" hidden></div>
          <p class="muted-note">Digite apenas o nome. O sistema gera o PIN, salva/atualiza o acesso no banco e monta a mensagem pronta para WhatsApp. Para remover alguém sem apagar histórico, use a tabela abaixo.</p>
        </form>
  ...
  function limparFormParticipante() {
    const idEl = $("#admin-participante-id");
    const nomeEl = $("#admin-nome");
    const box = $("#admin-acesso-gerado");
    if (idEl) idEl.value = "";
    if (nomeEl) nomeEl.value = "";
    if (box) { box.hidden = true; box.innerHTML = ""; }
  }

  function participantePorLogin(login) {
    const alvo = limparNomeParticipante(login).toLowerCase();
    if (!alvo) return null;
    return (state.participantes || []).find(p => String(p.login || "").toLowerCase() === alvo) || null;
  }

  function participantePorId(id) {
    if (!id) return null;
    return (state.participantes || []).find(p => String(p.participante_id || p.id) === String(id)) || null;
  }
  ...
  function preencherParticipante(id) {
    ...
    $("#admin-nome").value = p.nome || p.login || "";
    $("#admin-acesso-gerado")?.setAttribute("hidden", "");
  ...
  function mensagemErroParticipante(err) {
    ...
    if (/duplicate|unique|br_participantes_login|login/i.test(msg) && /existe|duplicate|unique|duplic/i.test(msg)) {
      return "Já existe participante com esse nome/login. Clique em editar na tabela ou aguarde a lista atualizar e tente renovar o acesso.";
    }
    if (/pin/i.test(msg)) {
  ...
  function mostrarAcessoGerado({ nome, login, pin, atualizado }) {
    const box = $("#admin-acesso-gerado");
    if (!box) return;
    const mensagem = mensagemWhatsappAcesso(nome, login, pin);
    box.hidden = false;
    box.innerHTML = `
      <div class="access-ok">✅ Acesso ${atualizado ? "renovado" : "criado"} no banco.</div>
      <div class="access-credentials"><span>Usuário: <strong>${escapeHtml(login)}</strong></span><span>PIN: <strong>${escapeHtml(pin)}</strong></span></div>
      <label>Mensagem para WhatsApp
        <textarea id="admin-msg-whatsapp" rows="8" readonly>${escapeHtml(mensagem)}</textarea>
      </label>
      <div class="actions"><button class="btn secondary" type="button" id="copiar-msg-acesso">📋 copiar mensagem para WhatsApp</button></div>
    `;
    $("#copiar-msg-acesso")?.addEventListener("click", () => copiarTexto(mensagem, "Mensagem de acesso copiada para o WhatsApp."));
    copiarTexto(mensagem, "Acesso salvo e mensagem copiada para o WhatsApp.");
  }

  async function salvarParticipanteAdmin(ev) {
    ev.preventDefault();
    try {
      const nome = limparNomeParticipante($("#admin-nome")?.value || "");
      if (!nome) throw new Error("Informe o nome do participante.");

      const login = loginDoNomeParticipante(nome);
      const idInformado = $("#admin-participante-id")?.value || null;
      const existente = participantePorId(idInformado) || participantePorLogin(login);
      const participanteId = idInformado || existente?.participante_id || existente?.id || null;
      const atualizado = Boolean(participanteId);
      const pin = pinAleatorio();
      const adminAtual = Boolean(existente?.admin);

      status(atualizado ? "Renovando acesso do participante..." : "Criando acesso do participante...", "warn");
      ...
      await renderAdmin();
      mostrarAcessoGerado({ nome, login, pin, atualizado });
